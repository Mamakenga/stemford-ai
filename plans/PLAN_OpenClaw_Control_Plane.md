...
Цель: снизить latency на самых частых управленческих командах без деградации качества ответов.

Текущая проблема:
1. Даже простые запросы ("покажи открытые задачи", "покажи pending approvals") идут через полный LLM-цикл.
2. Для MVP это приемлемо, но дает заметную задержку и нестабильное UX по сравнению с более "скриптовым" контуром.

Решение (отложенный backlog):
1. Ввести rule-based fast-path для фиксированного набора интентов:
- `покажи открытые задачи` -> прямой вызов `GET /tasks` с фильтром открытых статусов.
...
3. Все остальные запросы оставлять в обычном NL-пайплайне через OpenClaw + skill.
...
3. Для fast-path запросов сохраняется логирование действий и выборок в текущем audit-контуре.
4. При ошибке fast-path маршрут автоматически делает fallback в стандартный NL-пайплайн.
